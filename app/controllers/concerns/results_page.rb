# Shared "roster + NRR chart + renewal donut + global upsells" logic behind both "Résultats AM"
# (PilotageController) and "Résultats KAM" (ResultatsKamController) — identical structure, each scoped to
# a different role via #role_users, which the including controller must define. Reassignment targets
# (@reassignable_ams) are deliberately NOT role-scoped — any active user can be a reassignment target
# regardless of which results page you're reassigning from.
module ResultsPage
  extend ActiveSupport::Concern

  def show
    @active_ams = role_users
    @am_rows = @active_ams.map do |am|
      { am: am, summary: PortfolioSummary.new(am.companies.includes(:deals), user: am) }
    end
    @global_summary = PortfolioSummary.new(Company.where(user: @active_ams))
  ...
  # renewed are in scope — "En cours" and "Churné" fall outside this chart entirely. Scoped to this page's
  # role — "Résultats AM" only ever counts AM-owned companies, "Résultats KAM" only KAM-owned ones.
  def renewal_donut_slices
    scope = ProduitDeal.joins(:company).where(companies: { user_id: @active_ams.map(&:id) })
  ...
  # Upsells for companies owned by this page's role (not by effective_user — a produit-owning AM's company
  # stays listed here even if one of its upsells is currently reassigned to a KAM; the "AM" column just
  # shows who's actually working it).
  def filtered_global_upsells
    deals = UpsellDeal.joins(:company).where(companies: { user_id: @active_ams.map(&:id) })
      .includes(:user, company: :user).to_a
